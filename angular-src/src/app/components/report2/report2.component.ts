...
export class Report2Component implements OnInit {
  ...
  onGenerateReport(){
    ...
    if("undefined" !== typeof aYear.academicyear){
      this.authService.generateReportforSubmittedGradDates(aYear).subscribe(schooldata => {
        if(schooldata.success){
          this.schoolData = schooldata.schoolnamesandgraduationdatecountywise;
            for (var i = 0; i < this.schoolData.length; i++){
              ...
              for (var j = 0; j < this.schoolData[i].countschools; j++){
                ...
                var schoolName = this.schoolData[i].schoolwisegraduationdate[j].split(":");

                _lschools.push(schoolName[0]);
                _ldates.push(schoolName[1]);
              }
              l_county.push(this.schoolData[i]._id.countyname);
              l_schools.push(_lschools);
              l_dates.push(_ldates);
            }
  ...
  loadPDF() {
    ...
    // console.log('aYear; ' + academicyear);
  ...
  getRowsData() {
     ...
     for (let i = 0; i <rowCount; i++) {
       ...
       let graduationdates = this.g_dates[i];
       let schoolnamesFormatted = "";
       let graduationdatesFormatted = "";

       for (let j = 0; j < schoolnames.length; j++) {
         schoolnamesFormatted += schoolnames[j] + "\n";
       }
      //  console.log('asdasf-->' + schoolnamesFormatted);
       for (let k = 0; k < graduationdates.length; k++) {
         graduationdatesFormatted += graduationdates[k] + "\n";
       }

       schoolnamesFormatted = schoolnamesFormatted.substring(0, schoolnamesFormatted.length-1);
       graduationdatesFormatted = graduationdatesFormatted.substring(0, graduationdatesFormatted.length-1);
         data.push({
             id: i+1,
             county: this.g_county[i],
             schoolname: schoolnamesFormatted,
             graduationdate: graduationdatesFormatted
         });
     }
     return data;
   }
